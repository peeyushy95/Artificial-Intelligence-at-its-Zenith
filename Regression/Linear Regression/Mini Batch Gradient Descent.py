...
class Linear_Regression :
    ...
    def batch_gradient_descent(self):
        ...
        while not converged :
            tempError = sys.float_info.max
            tempBetaOne = 0
            tempBetaZero = 0
            
            for ind in range(0,n,batchSize):
                
                gradZero = 1.0/n * sum([ (self.betaZero + self.betaOne*self.x[i] - self.y[i]) for i in range(ind,batchSize + ind)])
                gradOne = 1.0/n * sum([ (self.betaZero + self.betaOne*self.x[i] - self.y[i])*self.x[i] for i in range(ind,batchSize + ind)])
