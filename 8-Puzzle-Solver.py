...
def DFS_Search(initial_state, goal_state) :
    ...
    while stack.isEmpty() == False :
        ...
        neighbour_states = explore_next_states(current_state, explored_states,stack.item())
        for states in neighbour_states:
            stack.push(states)
        if len(explored_states) % 10000 == 0 :
            print(len(explored_states))
    
            
    return False
    
def explore_next_states(current_state, explored_states,stack):
    
    new_states = []
        
    new_state = move('down', current_state)
    if new_state[0] == True and any(x for x in explored_states if x == new_state[1].config) == False and any(x for x in stack if x.config == new_state[1].config) == False:
        new_states.append(new_state[1])
        new_state[1].parent = current_state
        #print(new_state[1].config)
...
def move(direction,state):
    ...
    state_list = list(new_state.config)
    if direction == 'up':
    ...
    new_state.config = ''.join(state_list)
